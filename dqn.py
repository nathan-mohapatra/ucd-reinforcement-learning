...
def compute_td_loss(model, target_model, batch_size, gamma, replay_buffer):
    ...
    #q_update = reward + gamma * torch.max(q_values_next, dim=1)[0]
    #q_update *= (1 - done)
   
    #loss = torch.mean((q_values - q_update.detach()) ** 2)
    ...
    q_value_update = reward + gamma * q_value_next
    q_value_update *= (1 - done)

    loss = (q_value - Variable(q_value_update.data)).pow(2).mean()
